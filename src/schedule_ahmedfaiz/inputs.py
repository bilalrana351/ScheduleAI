...
def get_regular_tasks():
    ...
    while True:
        ...
        if not duration.isdigit() or int(duration) <= 0:
            ...
            continue
        priority = input(f"Priority for '{task}' (High/Medium/Low): ").lower()
        flexibility = input(f"Is '{task}' flexible? (yes/no): ").lower() == "yes"
        tasks.append({"task": task, "duration": int(duration), "priority": priority, "flexibility": flexibility})
    return tasks
