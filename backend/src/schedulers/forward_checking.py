...
def forward_checking_schedule(wake_up, sleep, obligations, tasks, rest_time=0):  # Keep parameter for compatibility
    ...
    for task in tasks:
        duration = task["duration"]  # Removed rest_time addition
        task_domains = []
        for slot in timeline:
            ...
            if available_minutes >= duration:
                ...
                task_end = (datetime.combine(datetime.today(), task_start) +
                          timedelta(minutes=duration)).time()  # Removed rest_time
                task_domains.append((task_start, task_end))
